...
abstract: |
  Google Sheets es una aplicación de hoja de cálculo gratuita y basada en la web provista por Google Drive. Está diseñada para la colaboración en tiempo real, permitiendo que múltiples usuarios editen simultáneamente. Su estructura básica se define por la Ce  lda, Fila y Columna, y los formatos básicos aseguran la legibilidad, incluyendo el formato Moneda (ej. €) y el uso de bordes.
    ...
    La lógica condicional se gestiona mediante la Referencia Absoluta ($) para fijar valores (ej. un Extra de 0,5 o el IVA del 21,00%), y la Función SI (IF), que automatiza decisiones (ej. mostrar Aprobado o Suspenso). El Formato Condicional aplica estilos automáticamente (ej. notas ≥5 en verde).
    Para el análisis avanzado, la función BUSCARV (VLOOKUP) busca valores (ej. la Densidad de Madrid: 841,17 hab/km²). La función QUERY permite operaciones SQL-like. La herramienta Explore utiliza machine learning (IA) para sugerir gráficos y responder preguntas en lenguaje natural.
